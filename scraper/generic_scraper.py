"""
汎用スクレイパー (GenericScraper)

config.yml に index_urls・base_url・municipality_id・name を書くだけで動く。
selectors や subsidy_keywords は省略可能（base_scraper.py のデフォルト値が使われる）。

新規自治体の最小 config.yml:
    municipality_id: yokohama
    name: 横浜市
    base_url: https://www.city.yokohama.lg.jp
    index_urls:
      - https://www.city.yokohama.lg.jp/business/kigyou-shien/

既存の自治体（scraper.py あり）はこのクラスを使わず従来通り動作する。
"""
import logging
...
from scraper.utils import html_parser, http_client, pdf_extractor

logger = logging.getLogger(__name__)


class GenericScraper(BaseScraper):
    """config.yml の値だけで動く汎用スクレイパー。"""
    ...
    def _fetch_html(self, url: str, cfg: dict, rate: float) -> RawSubsidy | None:
        ...
        # ページ内PDFリンクがあれば追記
        for pdf_url in self._find_pdf_links(resp.content, url)[:3]:
            try:
                pdf_bytes = http_client.get_bytes(
                    pdf_url, rate_limit=rate, max_bytes=cfg["pdf_max_bytes"]
                )
    ...
    def _fetch_pdf(self, url: str, cfg: dict, rate: float) -> RawSubsidy | None:
        try:
            pdf_bytes = http_client.get_bytes(
                url, rate_limit=rate, max_bytes=cfg["pdf_max_bytes"]
            )
        except Exception as e:
