...
class OsakaScraper(BaseScraper):
    ...
    def _crawl_index(self, index_url: str, cfg: dict, seen: set[str]) -> Iterator[str]:
        ...
        for page_num in range(1, max_pages + 1):
            url = f"{index_url}?{param}={page_num}" if page_num > 1 else index_url
            try:
                resp = http_client.get(url, rate_limit=rate)
            except Exception as e:
                logger.warning("インデックスページ取得失敗 url=%s err=%s", url, e)
                break

            links = html_parser.extract_links(resp.content, cfg["base_url"], selector)
            if not links:
                break
    ...
    def _fetch_html(self, url: str, cfg: dict, rate: float) -> RawSubsidy | None:
        ...
        for pdf_url in pdf_links[:3]:  # 最大3件
            try:
    ...
    def _fetch_pdf(self, url: str, cfg: dict, rate: float) -> RawSubsidy | None:
        ...
        if not text.strip():
            logger.warning("PDFテキスト抽出失敗（スキャン画像の可能性） url=%s", url)
            return None
