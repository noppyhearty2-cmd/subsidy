...
class OsakaPrefScraper(BaseScraper):
    ...
    def _is_municipality_administered(self, text: str) -> bool:
        """
        市区町村が申請窓口となる制度（府ページは解説のみ）を除外。
        申請は各市区町村へ、という制度は記事価値が低いためスキップ。
        """
        exclude_phrases = self.get_config().get("exclude_if_all", [])
        return all(phrase in text for phrase in exclude_phrases)
    ...
    def _fetch_html(self, url: str, cfg: dict, rate: float) -> RawSubsidy | None:
        ...
        if self._is_municipality_administered(text):
            logger.info("市区町村経由制度のためスキップ url=%s", url)
            return None
